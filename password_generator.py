...
'''
This API is used to generate the password.
If the user already exists in the provided system, then it will deny otherwise, will create a new user.
Note : This is system generated password
'''
...
def generate_password():
    ...
    if row.empty:
        ...
        # check if the username is provided or not. This is to avoid generating passwords without username
        if not username:
...
'''
This API is used to create the password.
Since the user will create the password so he has to match certain criteria (all_checks function is responsible for this)
If the user already exists in the provided system, then it will deny otherwise, will create a new user.
'''
...
'''
This API is used to renew the password.
User's password will be expired after 30 days which will enforce user to change it
User will be prompted to renew the password if changes made in the configuration criteria.
'''
...
def renew_password():
    ...
    df = read_df_from_csv(file_names('file_name'))
    row = df.loc[(df['Username'] == username) & (df['System'] == system)]
    # This will update the password of existing user i.e. update existing record.
    if row.empty:
        ...
        # checking if the user is trying to use the last password or not.
        response = match_password(hashed_password, password)
        if response == True:
            error = "Password cannot be same as of previous one"
            return render_template(renew_html, error=error),400
        # checking if the provided password is matching criteria or not.
        response = all_checks(password, confirm_password)
        if response:
            return render_template(renew_html,
                                   error=response),400
        # hashing the password after passing all the checks and then save the password
        hashed_password, salt = hash_password(password)
...
'''
Used by Admins
This is API is used to render admin to the PMS login page
'''
...
'''
Used by Users
This is API is used to render user to login
'''
...
'''
This API is used to validate the login credentials provided by admin
'''
...
'''
This API is used to validate the login credentials provided by users
'''
...
def user_login_validation():
    ...
    if row.empty:
        ...
        # This else block is used if any changes made to config file in terms of criteria change.
        else:
            password_min_chars = str(config.get('PASSWORD', 'CHAR_COUNT'))
            required_chars = str(config.get('PASSWORD', 'REQUIRED_CHARS').split(',')[3])

            # enforce user to change password if it has been more than 30 days since he generated the last password.
            ...
            #This will validate if the entered password satisfies the latest criteria otherwise enforce user to change it.
...
'''
This API is used to render user to the Create password or generate password page according to option he has chosen.
'''
